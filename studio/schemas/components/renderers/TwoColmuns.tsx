...
import { TwoColums } from '../../generated-schema';
...
const TwoColumns: React.FunctionComponent<TwoColumsProps> = ({
  ...
  if (!value) return <></>;
  const { dropColumnScreenSize, leftPercentage = '1/2' } = value;
  ...
  return (
    <div>
      <div className="block">
        <div
          className={`${dropClassName} ${responsiveAt}:w-${leftPercentage} pr-4`}
        >
          <RichText blocks={value.leftContent} />
        </div>
        <div
          className={`${dropClassName} ${responsiveAt}:w-${rightPercentage}`}
        >
          <RichText blocks={value.rightContent} />
        </div>
      </div>
